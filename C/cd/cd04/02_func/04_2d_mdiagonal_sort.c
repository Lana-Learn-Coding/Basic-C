...
void sort_mdiagonal_asc(int *ptr_arr, int rows, int columns);
...
int main(int argc, char const *argv[])
{
	...
	sort_mdiagonal_asc(ptr_arr, rows, rows);
	printf("duong cheo chinh duoc sap xep tu nho den lon");
...
void sort_mdiagonal_asc(int *ptr_arr, int rows, int columns)
{
	...
	if (is_square) {
		...
		for (i = 1; i < rows; i++) {
			int pre_i = i - 1;
			int *ptr_num = (i * rows + i) + ptr_arr;
			int *ptr_pre_num = (pre_i * rows + pre_i) + ptr_arr;
